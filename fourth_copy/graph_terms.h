		...
		{ 
			for (j = 0; j < length_y; j++)
			{
				...
				{   
					...
					fprintf_s(outputfile, "%d,%d,%d,%10.14lf,%10.14lf,%10.14lf,%lf \n", i, j, k, phi_A[i+j*length_y+k*area], phi_B[i+j*length_y+k*area], phi_C[i+j*length_y+k*area], lattice_height[i+j*length_y+k*area]);
...
{ 
	...
	printf("phi_A total is %10.14lf\n", phi_A_total/double(volume-2*(area)));	
	printf("phi_B total is %10.14lf\n", phi_B_total/double(volume-2*(area)));	
	printf("phi_C total is %10.14lf\n", phi_C_total/double(volume-2*(area)));
...
{ 
	...
	char phi_name1[FILENAME_MAX];
	char phi_name2[FILENAME_MAX];

	double z_max = length_z - 2;
	
	sprintf_s(phi_name1, "%s%d%s%d%s", "results/isoplot/phi_A/phi_counter_start_", counter_start, "_counter_", counter, ".txt");
	
	
	FILE *outputfile1;
	fopen_s(&outputfile1,phi_name1,"w");
		...
		{ 
			for (j = 0; j < length_y; j++)
			{
				...
				{   
					...
					fprintf_s(outputfile1, "%10.14lf,", phi_A[i+j*length_y+k*area]);
	...
	sprintf_s(phi_name2, "%s%d%s%d%s", "results/isoplot/phi_B/phi_counter_start_", counter_start, "_counter_", counter, ".txt");
		FILE *outputfile2;
	fopen_s(&outputfile2,phi_name2,"w");
		...
		{ 
			for (j = 0; j < length_y; j++)
			{
				...
				{   
					...
					fprintf_s(outputfile2, "%10.14lf,", phi_B[i+j*length_y+k*area]);
