...
__global__ void phi_surf(double *g_device_phi_A, double *g_device_phi_B, double *g_device_delta_z)
{
 ...
 //	unsigned int ks = blockIdx.y;
	...
	g_device_delta_z[ks_lower] = g_device_delta_z[ks_lower+4];
	g_device_delta_z[ks_lower+1] = g_device_delta_z[ks_lower+3];

	g_device_delta_z[ks_upper] = g_device_delta_z[ks_upper-4];
	g_device_delta_z[ks_upper-1] = g_device_delta_z[ks_upper-3];
					...
					double *g_device_delta_z)
{
	...
	unsigned int above = ((ks+1)&g_pbc_z);
    unsigned int below = ((ks-1)&g_pbc_z);

	
	double delta_z_ks = g_device_delta_z[ks];
	double delta_z_above = g_device_delta_z[above];
	double delta_z_below = g_device_delta_z[below];
...
// these values all need setting or passing into the function	
	...
	laplace_phi_A =  (((phi_A_left+phi_A_right)-(2.0*phi_A_is))/(g_delta_x*g_delta_x))
		            +(((phi_A_forw+phi_A_back)-(2.0*phi_A_is))/(g_delta_y*g_delta_y)) 
				//	+(((phi_A_up+phi_A_down)-(2.0*phi_A_is))/(delta_z_ks*delta_z_ks)); 
					+((4.0/(delta_z_ks+delta_z_below))*(((phi_A_up-phi_A_is)/(delta_z_ks+delta_z_above))-((phi_A_is-phi_A_down)/(delta_z_ks+delta_z_below))));

	double laplace_phi_B;
	laplace_phi_B =  (((phi_B_left+phi_B_right)-(2.0*phi_B_is))/(g_delta_x*g_delta_x))
		            +(((phi_B_forw+phi_B_back)-(2.0*phi_B_is))/(g_delta_y*g_delta_y)) 
				//	+(((phi_B_up+phi_B_down)-(2.0*phi_B_is))/(delta_z_ks*delta_z_ks)); 
					+((4.0/(delta_z_ks+delta_z_below))*(((phi_B_up-phi_B_is)/(delta_z_ks+delta_z_above))-((phi_B_is-phi_B_down)/(delta_z_ks+delta_z_below))));

				//+(((phi_B_up+phi_B_down)-(2.0*phi_B_is))/(((delta_z_above+delta_z_ks)*(delta_z_ks+delta_z_below))/4.0));
	...
	grad_phi_A_sq += ((phi_A_up-phi_A_down)*(phi_A_up-phi_A_down))/((delta_z_above+delta_z_below)*(delta_z_above+delta_z_below));
	...
	grad_phi_B_sq += ((phi_B_up-phi_B_down)*(phi_B_up-phi_B_down))/((delta_z_above+delta_z_below)*(delta_z_above+delta_z_below));
...
//	double delta_z_below = g_device_delta_z[below];
   ...
   //loads the tempory array phi_A with the current values
	...
	double grad_mu1_A_z = (device_mu1_A_up-device_mu1_A_is)/((delta_z_above+delta_z_ks)/2);
	...
	double grad_mu2_A_z = (device_mu2_A_up-device_mu2_A_is)/((delta_z_above+delta_z_ks)/2);
	...
	double grad_mu1_B_z = (device_mu1_B_up-device_mu1_B_is)/((delta_z_above+delta_z_ks)/2);
	...
	double grad_mu2_B_z = (device_mu2_B_up-device_mu2_B_is)/((delta_z_above+delta_z_ks)/2);
